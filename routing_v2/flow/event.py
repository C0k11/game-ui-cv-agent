...
class EventFlow(FormationMixin, BattleMixin, ExitMixin, Flow):
    ...
    #        ② bonus_sweep —— 扫荡同一关，把 AP 都花掉（自动套用①的纪录）
    #    ⛔只做①等于白打（AP 没花掉）；只做②等于用旧纪录刷（倍率低）。
    #      我第一版只写了①，是漏的。
    ...
    def _bonus_step(self, obs, st, rows):
        ...
        if ph == "bonus_clear":
            ...
            if self._bt()["win"] > base:
                ...
                if ti + 1 < need:
                    ...
                    self.log(f"✅加成①目标[{ti+1}/{need}] 纪录已顶 → 打下一个目标")
                    return wait("换下一个加成目标")
                self.state["phase"] = "bonus_sweep"
                self.log(f"✅加成①全部完成：部队{self.want_team} 打赢 "
            ...
            if self._bt()["unknown"] >= 3:
                return self.finish(
                    Outcome.BLOCKED,
                    f"⛔加成①打了 {self._bt()['unknown']} 场都没看到胜利横幅 —"
            ...
            return tap_box(target,
                           f"加成①进关顶纪录（要用部队{self.want_team}）",
        ...
        return tap_box(target,
                       f"加成②进关准备扫荡（{si+1}/{max(1,len(plan))} ← {why}）",
    ...
    def on_stage_popup(self, obs, st):
        ...
        加成阶段②要走**扫荡**分支：先把数量拉 MAX（游戏会自己钳到 AP 付得起
        的次数），再点扫荡开始。
