    ...
    def jit(self, act: Action, obs: Observation,
        ...
        if act.anchor is not None:
            ...
            if drift > tol:
                ...
                #      ↗ 等到了 `确认键`（确认框弹出来了）
                #      ⛔丢弃 '确认键' 锚点 y 0.985→0.842 位移 0.144 > 容差 0.029
                #      ⛔丢弃 '取消键' 锚点 y 0.845→0.803 位移 0.042 > 容差 0.028
    ...
    def arm(self, act: Action) -> None:
        ...
        ⭐⭐**默认契约是全局的，不用每处手工声明**（2026-08-12 用户点名:
           ...
           理由：点了什么，它就该消失或变样。实测覆盖面很广 ——
             購買→被确认框盖住 / 邀请卷→被面板盖住 / 设施入口→换页 /
             摸头气泡→消失 / `全部选择`→变成`已全部选择`（cls 名变了＝原 cls 没了）/
             `移动至2号点`→变成`移动至一号店` / 確認键→框关掉
