...
STRIP = {
    ...
    #      pad_max 收到 30.0。
    #    ⛔这一格读**低**的后果是"钱不够→不买"（漏做，无损）；读**高**才危险
...
#   「票数读不出→fail-closed」在有入场键时**根本不拦**，0 票也会去开面板。
...
class Vote:
    ...
    digit OCR 的不稳定是**双向**的（首位 DROP 6587→587，右界过读 999→9999），
