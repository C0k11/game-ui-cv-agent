...
Flow:
  ...
  verify   落点校验: quest列表特征(入场键+关卡得星) / Challenge tab切Quest /
           错活动(如情人節約定成就页: 全部领取_黄)顺手领back 重试
  survey   列表滑到底, 自底向上开尾部 N 关 popup, 记录 Bonus 解锁状态
  unlock   未解锁关: 任務開始编队(2部队快速編輯自動確認%读数)出击
            battle(AUTO gate)结算回列表
...
class EventQuestSkill(BaseSkill):
    ...
    def _init_state(self) -> None:
        ...
        # 共存(v14 训练没问题) — 检出405tap 落屏有 1-2s 延迟, 点下去时轮播
    ...
    def parse_quest_rows(self, screen: ScreenState) -> List[Dict[str, Any]]:
        ...
        **绝不允许退回用 cy 当身份**(那正是要消灭的东西)。
        ...
        # 自上而下必须严格递增。相邻误读(1112)在 1-30 范围闸内完全合法, 一旦
    ...
    def _enter(self, screen: ScreenState) -> Dict[str, Any]:
        ...
        目标 cls(405)出现立即点; 加载中wait; 无锁定wait(轮播在转)。"""
        ...
        if banner is None and not fresh_usable:
            ...
            # 检出405tap 落屏延迟 >1.5s 就点到切页后的 474 卡。策略=检出即
            ...
            if b405 is not None:
                ...
                # 2.5s/页: 检出tap 延迟 >1.5s 会点到切页后的 474 卡(误入
                ...
                # 「检出405tap 落屏延迟 >1.5s 就点到切页后的 474 卡(误入三连根因)」
    ...
    def _verify(self, screen: ScreenState) -> Dict[str, Any]:
        ...
        if ev_marker is not None and self._phase_ticks <= _PHASE_MAX * 2:
            # 上期活动陷阱(2026-07-22 三连实锤): 检出405tap 延迟撞轮播
        ...
        if self._popup_open:
            if self._on_popup(screen):
                ...
                if all(abs(self._survey_cy - c) > 0.03
                    ...
                    # Bonus xN ≥ 80% 固定 xN 才算加成打满; 读不出保守按
    ...
    def _unlock(self, screen: ScreenState) -> Dict[str, Any]:
        ...
        # 吞(scrcpy 断流 ADB 帧源翻转帧未稳定), stage 已进 "edit"  弹窗仍
        ...
        # 重试窗), 两个闸极性相反。_read_apNone 是已被 live 反复证实的常态
        ...
        if step == "sortie":
            ...
            if sortie is not None:
                ...
                # 只在 _formation_step=="" 评估一次, 之后经过 editautoconfirm
