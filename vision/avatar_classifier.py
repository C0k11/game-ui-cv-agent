...
# ---- model location ---------------------------------------------------------
# v2 (2026-05-18): 267-class model trained on trajectory + 角色头像_crop refs.
# Trajectory val: 95.91% top1 (up from v1's 86.55%).  Class coverage went
# from 29 (trajectory-only) to 267 (every BA student with a local ref).
DEFAULT_MODEL_PATH = Path(
    r"D:\Project\ml_cache\models\yolo\runs\avatar_cls_v2_yolo26n\weights\best.pt"
)
# Legacy v1 fallback (29 classes, trajectory-only).
LEGACY_MODEL_PATH = Path(
    r"D:\Project\ml_cache\models\yolo\runs\avatar_cls_yolo26n\weights\best.pt"
)
# Bilingual class name file — used to render "Wakamo (若藻)" in logs so the
# operator can scan-read whose avatar the model identified.
BILINGUAL_FILE = Path(
    r"D:\Project\ai game secretary\data\avatar_cls_names_bilingual.json"
)
...
class AvatarClassifier:
    ...
    def __init__(
        ...
        # EN-only-class -> Chinese name lookup for human-readable logs.
        # Loaded from data/avatar_cls_names_bilingual.json (generated by
        # scripts/build_avatar_cls_v2_dataset.py + name_map).
        self._en_to_cn: Dict[str, str] = {}
        if BILINGUAL_FILE.exists():
            try:
                raw = json.loads(BILINGUAL_FILE.read_text(encoding="utf-8"))
                self._en_to_cn = {v["en"]: v["cn"] for v in raw.values() if v.get("cn")}
            except Exception:
                pass
        # vote buffer: (room_idx, cell_idx) -> deque of (probs np.ndarray, frame_id)
        self._vote_buffer: Dict[Tuple[int, int], deque] = {}

    def label(self, en_name: Optional[str]) -> str:
        """Render a class name with its CN equivalent if known.

        'Wakamo' -> 'Wakamo(若藻)';  'Rio' -> 'Rio(理緒)';
        unknown classes returned as-is.
        """
        if not en_name:
            return str(en_name)
        cn = self._en_to_cn.get(en_name)
        return f"{en_name}({cn})" if cn else en_name
    ...
    def _dump_unknown(
        ...
        try:
            ...
            # Encode the model's best guess with its CN name (if known) so the
            # operator can tell at a glance whose avatar this might be.
            cn = self._en_to_cn.get(guess)
            tag = f"{guess}_{cn}" if cn else guess
            safe_guess = tag.replace("/", "_").replace("\\", "_")
