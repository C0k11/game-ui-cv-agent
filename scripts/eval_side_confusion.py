...
**分母必须拆两套报, 只报一套会骗人**(2026-07-25 实测撞到):
  v10 在 holdout 上"敌友 5/68=7.4%"看着比全池的 22.5% 好得多, 真相是
  **召回只有 43.8%** —— 大部分敌人压根没检出, 没检出就无从误分类。
   一个什么都不检的模型能拿 0% 误分类。所以:
    敌友(全)  = GT敌方被判我方 / **全部 GT 敌方框**    端到端损害
    敌友(匹配)= GT敌方被判我方 / **匹配上的 GT 敌方框**  纯分类错误率
