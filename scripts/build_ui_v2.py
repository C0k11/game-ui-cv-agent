...
REAL_SOURCES += _clean_dirs(FLYWHEEL_TRAIN_DAYS) + [
    ...
    #      拼图过了人审; 每帧再用模型 conf>=0.50 整帧预标打底防负监督。
    # 同批的"购买/购买灰色补强"**已整批撤回**(2026-08-13 当晚):
    #    那批"提案"是 scripts 用 detect.infer 生成的 = **模型自己的输出**,
    #    写成标签就是拿模型喂模型(自举偏差), 与历史上被拦下的 7,057 帧
    #    v14 预标同性质。2,271 帧已删标退回未标注态(备份见
    #    data/raw_images/_backups/20260813_emit_rollback), 12 个纯 MONEY
    #    抓拍目录因此清空、不再进源。
    #    要真正补强 购买/购买灰色, 只能靠 dashboard 人工画框, 不能靠
    #      "把模型低分检出提升成真值"。
