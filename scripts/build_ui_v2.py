...
REAL_SOURCES += _clean_dirs(FLYWHEEL_TRAIN_DAYS) + [
    ...
    "v2alt3_20260812",
    # -- 2026-08-13 锁类+购买补强批（emit_labels.py 落标, 拼图人审过）--
    #    锁 cls50 泛化: 大厅制造锁/社团卡锁/咖啡厅锁钮/任务大厅5种tile锁/
    #      创建日活動一覽锁/战斗AUTO横幅锁(此前 cls50 只有课程表一个域,
    #      lobby/club/cafe/task_hall 四域 v16 召回实测全 0%)。
    #    购买/购买灰色域补强: 本期活动商店皮肤(价签在按钮上方/售罄不变灰的
    #      第三形态)+信用点商店弱域低分样本。
    #    帧 = 整帧模型预标(conf>=0.50)打底+人审提案框, 待 dashboard 人审;
    #    注意 MONEY 抓拍帧近邻重复度高(055226 尤甚), dashboard 里按唯一帧看。
