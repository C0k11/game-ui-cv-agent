...
TRAIN_CONFIGS = {
    ...
    "battle_yolo26s_v8": {
        ...
        "base": str(YOLO_ROOT / "runs" / "battle_yolo26s_v7" / "weights" / "best.pt"),
        "epochs": 150,
        "patience": 40,
        "save_period": 10,
        "imgsz": 960,
        "batch": 16,   # batch32 在 batch0 炸 [17,5]张量错(未验证过的变量), 退回 v5 验证配方
        "out_name": "battle_yolo26s_v8",
        "cache": True,
        "workers": 8,
        "mosaic": 0.3, "close_mosaic": 15, "copy_paste": 0.0, "mixup": 0.0,
        "scale": 0.2, "translate": 0.1,
    ...
    "ui_yolo26m_v16": {
        ...
        "cache": False,
        # workers 固定 8。试过 14, DataLoader worker 直接崩:
        #   RuntimeError: Couldn't open shared file mapping
        # 32G 系统内存撑不住那么多 worker 的共享内存。
        # 注: workers=8 时 GPU 82% / 277W 是 YOLO 训练的正常负载, 不是喂不饱
        # (4090 的 450W 是推理/游戏满载的数, 训练达不到), 别照着功耗去调 workers。
        "workers": 8,
        "lr0": 0.005,
        "weight_decay": 0.0005,
        "dropout": 0.0,
