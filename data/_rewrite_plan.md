...
## OK 实现进度 (2026-06-02, commit 3b0a176->c1dfbbc)
全部 11 个新 skill 已**写完+编译+接 pipeline+整管线实例化通过**：
1. OK buy_pyroxene(新) 2. OK mail(重写) 3. OK craft(重写) 4. OK club(重写)
5. OK shop(重写,动态OCR预算) 6. OK bounty(重写->TicketSweepSkill基类)
7. OK jfd(新->TicketSweepSkill) 8. OK arena(重写,全自动PVP) 9. OK daily_mission(新,取代daily_tasks)
10. OK momo_talk(重写) 11. OK story_mining(重写,含主线3级钻取)
...
- 注意️ story 主线可能含**战斗节点**(无剧情menu)->现靠cutscene timeout退出,v6 加战斗节点处理。
- 注意️ 刷体力(AP farming)+活动(event) 整块**已移除**,未来需要再据新 probe 重写(非复活旧代码)。
...
## 文件处理
- OK 保留：`base.py`(BaseSkill+action_click/click_cls/find_cls/action_*) · `ui_classes.py`(cls名) ·
...
- **buy_pyroxene**：大厅`购买青辉石`(retry)->商店->`组合包未选择`tab->找`免费`cls->点其正下`购买`->确认框(验`免费`)->`确认键`->`获得奖励`(点头部框)->红点清=done。禁左右CAD真钱包
- **club(社交)**：大厅`社交入口`->`社團`卡(点卡身~0.235,0.52非红点,浮层点空白会收起)->自动弹簽到->`确认键`->done(10AP进信箱)
- **craft**：大厅`制造入口`->`一次领取黄色`(可领则领,`灰`跳过)->`快速制造`->`MAX_可点击`(在则点)->`开始制造`->确认框->done。禁`立即完成`(耗製造券)；`一次領取`若弹"立即完成"带券/石->取消
- **shop**：大厅`商店入口`->一般tab->`全部選擇`->`选择购买`->确认框【digit-OCR 持有&總價】持有≥總價+buffer则`确认键`否则`取消键`->`获得奖励`->done。禁`青輝石`tab
...
- **daily_mission(每日领奖)**：大厅左`每日领奖`(0.045,0.358≠任务大厅hub)->任務(全體tab)->循环`全部领取_黄`->reward(点击继续)直到灰->再扫单个`领取_黄`(meta任务)->done。**最后跑**(其他日常完才解锁)。禁`立即前往`未完成任务不碰
