...
| 池组 | 去向 | 说明 |
|---|---|---|
| `defeat_candidates_v10` (28帧/28框 cls484 战斗失败, **已人审OK**) | **battle v10** | 用户已标, 无需再审; v10 build SRCS 加此池, nc 18->19 |
...
| `flywheel_event_shop_20260717` (77帧, 未稀疏保全) | **ui v14** | 购买按钮/价格条/貨幣tab = v13 盲区解药(screen_flow 实锤3)), 预标必稀疏, **人审逐帧补框** |
...
- 其余快速过目纠错(预标底子好)
- 禁Challenge 上任何语义框一律删(见否决项)

**2)战斗材料独立池(battle v10 SRCS 按池加, 不合并)**:
- `defeat_candidates_v10`(28帧 cls484, 已人审OK 免审)
- `run_20260715_*_botplay_clean` ×7(498帧): DEFEAT 口径对齐/瞄准态/技能卡抽查/战斗胜利帧留意 val
  禁**2026-07-20 撤出 v10**(用户抓): battle 域框=battle v9 自己预标未过人审, 自蒸馏固化误检风险,
  差点混进 v10 训练(已发车 10ep 被叫停回炉)。**仍待人审, 审完进 v11**。v10 终版=v9九池+defeat28(train2001/val353)。
...
v14 发车前把本轮新定的标注规矩(灰态不标/405-474 判定键/103 底锚小框/Challenge 不标 80)
反向套到全部 ~80 个存量池(OCR 8并发 + HSV 8并发, 全部 dry-run+目检拼图后 apply):

- **0521 池 103 大框收缩 ×504**: 老框 0.0825×0.0776(圈价格+整钮) -> OCR 定位「購買」字底锚
  0.0353×0.0318, 与全网颗粒度归一。同池 101/102/104 语义与新规一致(黄字=已选择), 免改。
...
- **灰态清理**: 103 del ×109(0521 灰字淡蓝 48 + 各池遮罩暗态 61, 目检拼图确认);
