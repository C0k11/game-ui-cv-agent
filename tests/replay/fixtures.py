...
禁定位: 这是**把已修 bug 钉死不复发**的工具, 不是发现新 bug 的手段, 更不是
...
#  3) schedule 金钱防线 禁veto
# 注意原计划是拿"5 区 tickets=-1"的真实录像帧建 fixture, **建不出来**: Schedule
...
#   2) 弹窗 body 出现青辉石 -> 取消, 绝不点確認
#   3) 单日派遣硬上限 _MAX_TICKETS=7, 到顶什么都不点(不是"到顶还点确认")
...
def fx_event_unlock_never_confirm_buy_ap():
    """禁2026-07-25 全仓金钱审计 #1/#2: unlock 链上三处 確認 点击原本**零金钱闸**
...
def fx_event_unlock_ap_gate_fail_closed():
    ...
    for _ in range(int(eq._AP_READ_RETRY_SEC) + 30):
        ...
        if sk.sub_state != "unlock":
            break
    # 注意原版把迭代次数当秒数用: 紧凑调用只耗毫秒, 6s 重试窗内全是 wait,
...
def fx_page_graph_sweep_panels_not_confused():
    """禁悬赏/交流会/活动 三家扫荡面板 cls 集合几乎一样, 只有票据不同。
...
def fx_arena_never_reached_not_complete():
    """禁2026-07-25 假成功回归(run_20260725_231337 帧实锤):
    ...
    钉死两件事: 1)没到达过就绝不许 reason 里出现 complete 2)exit_report 三态正确。
    """
    from brain.skills.arena import ArenaSkill
    bad = []
    a = ArenaSkill(); a.reset()

    # 1) 没到达 -> done 的 reason 必须是 timeout 而不是 complete
    class _Scr:
        yolo_boxes: list = []
        frame = None
    a._goto("exit")
...
def fx_bounty_zero_tickets_never_overread():
    ...
    禁旧 strip 把它读成 `'9/0'` -> `_read_tickets` 返回 **9** -> `_ticket_check`
...
def fx_craft_empty_slots_still_enter():
    """禁制造红点门控的**第三态**: 三个槽位全空 -> 同样没有红点 -> 旧码永远 skip。

    2026-07-28 帧实锤(data 帧: 材料清單三行全是「＋開始製造」, 一次領取灰):
    旧 should_run 只认两态 1)有红点=可领 2)无红点=还在造->skip。第三态「根本没在
...
def fx_momo_unknown_dialog_never_confirm():
    ...
    **「點擊確認時將前往活動頁面」** —— 点確認就把 bot 带出 MomoTalk。
    ...
    # 1) 劇透警告框: 双钮 @cy0.913, 屏上无 story chrome
    warn = _sched_screen(
    ...
    for _ in range(4):
        ...
        if act.get("action") == "click":
            if "取消" not in r:
                ...
                cancels += 1
    # 禁连发也要钉: 这道闸第一版就漏了 after-ack, 上线第一次 live 就被 step_walk
    # 逮到「取消连发 3 次」(第 3 发时框已关掉, 拍在面板下方空白上)。
    if cancels != 1:
        bad.append(f"取消发了 {cancels} 次(期望 1, 其余该是 after-ack wait)")

    # 2) 略過剧情框: 双钮 @cy0.724 + 右上角 story chrome -> 必须还能確認
...
def fx_cafe_claim_earnings_no_premature_done():
    ...
    本 fixture 钉两件事: 1)点击那一刻 _earnings_done 必须仍为 False
                        2)同一帧再来一次必须是 after-ack wait, 不许连发
    """
    from brain.skills.cafe import CafeSkill
...
def fx_cafe_dot_gate_only_on_lobby():
    ...
    **3 个 Emoticon_Action(3 个学生等着摸头)**。
    ...
    # 1) 咖啡厅内部: 有 咖啡厅收益/移动至2号点/返回键, 没有大厅底栏
...
CASES = [
    ("禁cafe大厅门控_只在大厅判", fx_cafe_dot_gate_only_on_lobby, True),
    ("禁cafe领收益_点时绝不报领完", fx_cafe_claim_earnings_no_premature_done, True),
    ("禁cafe换2F_点前绝不改状态", fx_cafe_switch_2f_no_mutate_before_ack, True),
    ("禁cafe判dry_帧数够也要等墙钟", fx_cafe_dry_gate_needs_wallclock, True),
    ("禁momo未知確認框_只能取消", fx_momo_unknown_dialog_never_confirm, True),
    ("禁momo面板是别人时_一个click都不许发", fx_momo_panel_identity_gate, True),
    ("禁制造槽位全空_无红点也要进", fx_craft_empty_slots_still_enter, True),
    ("禁arena未到达_绝不报完成", fx_arena_never_reached_not_complete, True),
    ("禁shop留店标志跟随plan", fx_shop_chain_flag_follows_plan, True),
    ...
    ("禁买票框_绝不確認", fx_schedule_buy_dialog_never_confirm, True),
    ("禁票到顶_零点击", fx_schedule_ticket_cap_stops, True),
    ("禁真实事故帧_购买框无青辉石cls", fx_schedule_buy_dialog_no_pyroxene_cls, True),
    ("禁unlock链_購買AP框绝不確認", fx_event_unlock_never_confirm_buy_ap, True),
    ("禁unlock_AP读不出_fail-closed", fx_event_unlock_ap_gate_fail_closed, True),
    ("关popout不连发(after-ack)", fx_schedule_popout_close_no_double_fire, False),
    ("L12)_关号解析Q08-12", fx_quest_rows_numbers, True),
    ("L12)_旧cy主键台账必作废", fx_bonus_ledger_rejects_cy_keys, True),
    ...
    ("禁悬赏零票_绝不读成非零", fx_bounty_zero_tickets_never_overread, True),
    ("禁掃蕩确认框_绝不当成完成弹窗", fx_sweep_confirm_not_mistaken_for_done, True),
    ("课程表票数_cls锚定读得出", fx_sched_tickets_cls_anchored, False),
    ("禁课程表票_绝不吃「3->2」诱饵", fx_sched_ticket_decoy_never_read, True),
    ("禁面板底部假確認_不当对话框", fx_event_panel_fake_confirm_not_a_dialog, True),
    ("禁多关轮转_配额不MAX", fx_multi_farm_quota_not_max, True),
    ("禁错弹窗必关_吞发不解武装", fx_sweep_wrong_popup_must_close, True),
    ("禁弹窗身份读不出_fail-closed关窗", fx_sweep_unknown_popup_fail_closed, True),
    ("单关_仍走MAX", fx_single_farm_still_max, True),
    ("禁面板亮stepper_不当购买框", fx_panel_stepper_not_purchase_dialog, True),
]
...
def run() -> int:
    ...
    for name, fn, veto in CASES:
        ...
        mark = "PASS" if ok else ("FAIL禁" if veto else "FAIL")
        print(f"  [{mark}] {name}\n         {detail}")
        if not ok:
            fails += 1
            if veto:
